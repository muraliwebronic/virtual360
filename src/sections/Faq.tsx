...
export default function Faq() {
  ...
  const faqData: FaqItem[] = [
    ...
    },
    {
      ...
      answer:
        "Yes. On the Agency and Enterprise plans, you can remove the 'Powered by Webronic' branding and add your own logo or your client's logo to the tour player.",
    },
  ];
              ...
              {faqData.map((fq, index) => {
                ...
                return (
                  <div
                    ...
                    ${isOpen ? "border-[#2A74ED] ring-1 ring-[#2A74ED]/20" : "border-gray-200"}`}
                    aria-expanded={isOpen}
                  >
                    <div className="flex items-center justify-between gap-4">
                      <h3 className={`font-semibold text-lg ${isOpen ? "text-[#2A74ED]" : "text-[#1A1A1A]"}`}>
                        {fq.question}
                      </h3>
                      <div className={`${isOpen ? "text-[#2A74ED]" : "text-gray-400"} flex-shrink-0`}>
                        <Plus
                          className={`${isOpen ? "rotate-45" : "rotate-0"} transition-transform duration-300 ease-in-out`}
                          size={20}
                        />
                      </div>
                    </div>
